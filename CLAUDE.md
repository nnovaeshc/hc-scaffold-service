...
- **skill-creator triggers (interactive only; harness does not grade activation):** re-run when the frontmatter `description` in `SKILL.md` changes, when `skills/hc-scaffold-service/evals/triggers.json` changes, after a catalog-driven trigger-phrase sync, or when real use shows false positive/negative activation. Tell the user; print `task test:skill-creator:install` then `task test:skill-creator:triggers` and wait for them to confirm (or skip with a reason). Skip when only skill body / harness / docs changed. Full rules: [docs/triggers.md](docs/triggers.md).
- `task test:skill-creator:eval` is optional secondary behavioural eval via skill-creator; the stub harness remains primary for post-activation behaviour.
...
- [docs/triggers.md](docs/triggers.md) — frontmatter activation / when to re-run skill-creator triggers
